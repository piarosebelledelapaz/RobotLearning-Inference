...
POLICY_PATH="${POLICY_PATH:-artifacts/policies/mixed_010000/checkpoints/010000/pretrained_model}"
...
DATASET_PREFIX="${DATASET_PREFIX:-eval_towel_folding_challenge}"
...
if [ ! -d "$POLICY_PATH" ]; then
  ...
  echo "Download it first with: bash download_mixed_010000_challenge.sh" >&2
  exit 1
fi
...
for rollout in $(seq 1 "$NUM_ROLLOUTS"); do
  ...
  printf "\n=== Rollout %s/%s: running challenge policy ===\n" "$rollout" "$NUM_ROLLOUTS"
